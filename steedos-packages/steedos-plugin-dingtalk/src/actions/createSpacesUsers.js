...
module.exports = {
    handler: async function (ctx) {
        ...
        try {
            if (spaceDoc) {
                // 根据企业微信userid查询user
                let userDoc = await usersObj.findOne({ filters: [['dingtalk_id', '=', user.userid]] });
                let spaceUserDoc = await spaceUsersObj.findOne({ filters: [['dingtalk_id', '=', user.userid],['space', '=', spaceDoc._id]] });

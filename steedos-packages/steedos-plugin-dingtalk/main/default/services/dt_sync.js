...
exports.userinfoPush = async function (userId, status = 0) {
    try {
        var profile, user_email, userRes, profile;
        ...
        write("userId:" + userId)
        const userinfotRes = await dtApi.userGet(access_token, userId);
        ...
        for (let i = 0; i < userinfotRes['department'].length; i++) {
            const deptRes = await queryGraphql('{\n  organizations(filters: [[\"dingtalk_id\", \"=\", \"' + userinfotRes['department'][i] + '\"]]) {\n    _id\n    name\n  }\n}');
            deptIdList.push(deptRes['organizations'][0]['_id'])
        }

        userRes = await queryGraphql('{\n  space_users(filters: [[\"dingtalk_id\", \"=\", \"' + userId + '\"]]) {\n    _id\n    name\n  profile\n}\n}');
        let manage = userinfotRes['managerUserid'] == undefined ? "" : userinfotRes['managerUserid'];
        if (manage != "") {
            const manageRes = await queryGraphql('{\n  space_users(filters: [[\"dingtalk_id\", \"=\", \"' + manage + '\"]]) {\n    _id\n    owner\n  }\n}');
        ...
        const doc = '{user_accepted:true,organizations:' + userinfo['organizations'] + ',name:\"' + userinfo['name'] + '\",profile:\"' + userinfo['profile'] + '\",mobile:\"' + userinfo['mobile'] + '\",organization:\"' + userinfo['organization'] + '\",email:\"' + userinfo['email'] + '\",job_number:\"' + userinfo['job_number'] + '\",position:\"' + userinfo['position'] + '\",manager:\"' + userinfo['manage'] + '\",dingtalk_id:\"' + userinfo['dingtalk_id'] + '\"}';
        if (userRes.space_users.length == 0) {
            const insertUserRes = await queryGraphql('mutation {\n  space_users__insert(doc: ' + doc + ') {\n    _id\n  }\n}')
        } else {
            const updateUserRes = await queryGraphql('mutation {\n  space_users__update(id:\"' + userRes['space_users'][0]['_id'] + '\",doc: ' + doc + ') {\n    _id\n  }\n}')
        }
    } catch (error) {
        if (error){
...
exports.deptinfoPush = async function (deptId, status = 0) {
    try {
        var parent_id, parentDeptInfo, deptRes;
        ...
        write("access_token:" + access_token)
        write("deptId:" + deptId)
        const deptinfotRes = await dtApi.departmentGet(access_token, deptId);
        write(deptinfotRes)
        write("================获取部门详情 END===================")
        write("access_token:" + access_token)
        //查看数据库是否存在
        ...
        if(deptId.toString() == "1"){
            parent_id = null;
            // 更新根组织
            const updateDeptRes = await queryGraphql('mutation {\n  organizations__update(id:\"' + parentDeptInfo['organizations'][0]['_id'] + '\",doc: {dingtalk_id :\"' + deptId + '\",name: \"' + deptinfotRes['name'] + '\"}) {\n    _id\n  }\n}')
            return false;
        }else{
        ...
        if (deptRes.organizations.length == 0) {
            if (parent_id) {
                const insertDeptRes = await queryGraphql('mutation {\n  organizations__insert(doc: {dingtalk_id :\"' + deptId + '\",name: \"' + deptinfotRes['name'] + '\", parent: \"' + parent_id + '\"}) {\n    _id\n  }\n}')
            } else {
                const insertDeptRes = await queryGraphql('mutation {\n  organizations__insert(doc: {dingtalk_id :\"' + deptId + '\",name: \"' + deptinfotRes['name'] + '\"}) {\n    _id\n  }\n}')
            }
        } else if(parent_id){
...
exports.useridPush = async function (access_token, mobile) {
    try {
        ...
        write("access_token:" + access_token)
        write("mobile:" + mobile)
        const userinfotRes = await dtApi.userGetByMobile(access_token, mobile);
        // console.log("userinfotRes: ", userinfotRes);
        write(userinfotRes)
        write("================获取用户详情 END===================")

        const userRes = await queryGraphql('{\n  space_users(filters: [[\"mobile\", \"=\", \"' + mobile + '\"]]) {\n    _id\n    name\n  profile\n}\n}');
        ...
        const doc = '{dingtalk_id:\"' + userinfo['dingtalk_id'] + '\"}';
...
async function queryGraphql (queryStr) {
    ...
    const userSession = await objectql.broker.call("@steedos/service-accounts.getUserSession", {
        'authorization': 'Bearer apikey,' + API_KEY
    })

    const res = await objectql.broker.call('api.graphql', {
        query: queryStr
    },
        {
            meta: {

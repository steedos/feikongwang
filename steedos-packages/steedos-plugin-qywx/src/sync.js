...
const LOG_PATH = process.env.QYWX_LOGPATH || './qywx_server.log';
...
exports.deptinfoPush = async function (deptId, name, parentid, status = 0) {
  ...
  if (!name) {
    ...
    write("deptId:" + deptId)
    deptinfotRes = await fetch("https://qyapi.weixin.qq.com/cgi-bin/department/get?access_token=" + access_token + "&id=" + deptId);
    deptinfotRes = (await deptinfotRes.json()).department
    write(deptinfotRes)
  }
  write("================获取部门详情 END===================")
  //查看数据库是否存在
